...
leader(Id, Master, N, Slaves, Group) ->
 ...
 % a message either from its own master or from a peer node
        {mcast, Msg} ->
            ...
            bcast(Id, {msg,N, Msg}, Slaves, self()),
            Master ! Msg,
...
%and the process identifier of its group process.
        {join, Wrk, Peer} ->
           % io:format("leader 2 \n"),
            ...
            bcast(Id, {view,N, [self()|Slaves2], Group2}, Slaves2 , self()),
            Master ! {view, Group2},
...
bcast(_, _ , [] ,_) ->
    io:format("hej \n"), 
    ok;
bcast(Id, Message, [H | T], Leader) ->
    io:format("leader ~p H ~p \n" , [Leader , H]),
    H ! Message,
    crash(Id),

    case H of
        Leader -> bcast(Id , Message, T, Leader);
        [] -> bcast(Id , Message, T, Leader);
        _ -> wait_for_acc(Message , H , T,Id , Leader, 0)
   
    end.

   % self() == Leader -> wait_for_acc(Message , H , T,Id , Leader);
 
    %bcast(Id , Message, T).


wait_for_acc(Message , H, T, Id, Leader, Counter) ->
    io:format("wait for acc ~p   ~p \n" , [H, T]),
    receive 
        acknowledgement ->
        io:format("got acc \n"),
        bcast(Id , Message, T, Leader)   
    after ?timeout ->
        ...
        Counter2 = Counter + 1,
        H ! Message,
        wait_for_acc(Message , H, T, Id, Leader , Counter2)
end
end.
...
election(Id, Master, N, Last, Slaves, [_|Group]) ->
    ...
    case Slaves of
        ...
        %da blir man leader 
        [Self|Rest] ->
            bcast(Id, Last, Rest, Self),
            bcast(Id, {view,N, Slaves, Group}, Rest, Self),
            Master ! {view, Group},
